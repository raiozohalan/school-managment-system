<?php
	...
	if(!empty($_FILES["picture"]["name"])){
		...
		//	if($_POST["id_number"] == $_POST["old_idnumber"]){
	    //	$err+= 0;
		//	$picture= "";
		//}else{
			if(rename("../../".$_POST["old_picture"],$dir.$_POST["id"].".".pathinfo($_POST["old_picture"],PATHINFO_EXTENSION))){
				$err+=0;
				$picture = ",picture='images/student-pictures/".$_POST["id"].".".pathinfo($_POST["old_picture"],PATHINFO_EXTENSION)."'";
			}else{
				$err+= 1;
				$picture= "";
				$uploadErr = '<p class="w3-red w3-padding-medium w3-margin-0"><i class="fa fa-circle"></i> Error while renaming your picture</p>';
	...
	if($err == 0){
		// check if user exist
		$query = $con->query("select count(tbl_id)as rowCount from tbl_students where username='".$_POST["username"]."' or fname='".$_POST["fname"]."' and mname='".$_POST["mname"]."' and lname='".$_POST["lname"]."'");
		$row = $query->fetch();
		if($row["rowCount"] == 1 || $row["rowCount"] == 0){
				...
				<div id="messageModal" class="w3-modal w3-animate-zoom w3-padding-0" style="background:rgba(0,0,0,0.90);display:block;"> 
					<div class="w3-modal-content w3-card-8 w3-animate-zoom w3-round w3-border w3-border-white w3-padding-0" style="max-width:600px;margin-top:200px;overflow:hidden;">
						...
						<div class="w3-padding-medium w3-white w3-col s12 m12 l12">
							...
							<button class="w3-btn w3-padding-medium w3-grey w3-section w3-round" type="button" onclick="window.location.assign('update-profile.php')"><i class="fa fa-close"></i> Close</button> 
						</div>
					</div>
				</div>  
				<?php
...
<div id="messageModal" class="w3-modal w3-animate-zoom w3-padding-0" style="background:rgba(0,0,0,0.90);display:block;"> 
	<div class="w3-modal-content w3-card-8 w3-animate-zoom w3-round w3-border w3-border-white w3-padding-0" style="max-width:600px;margin-top:200px;overflow:hidden;">
		...
		<div class="w3-padding-medium w3-white w3-col s12 m12 l12">
			<p><i class="fa fa-warning w3-xlarge"></i> Error</p>
			<?php
