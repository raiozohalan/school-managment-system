...
<?php
	...
	if(!empty($_POST["search"])){
		$search = $_POST["search"]; 
			$searchtxt = " and subject_title like '$search%'"; 
	}else{
		$searchtxt = "";
	}
	$query_subjects = $con->query("select * from tbl_subjects where faculty_id='".$_SESSION["faculty_id"]."' $searchtxt order by subject_title ASC limit $limit");
	$count = 0;
