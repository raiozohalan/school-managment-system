<?php
	...
	$msg = "";
	$query_select  = $con->query("select count(tbl_id)as rowCount from tbl_news_and_events where title='".$_POST["title"]."' and tbl_id !='".$_POST["tbl_id"]."'");
	...
	if($row_select["rowCount"] == 0){
		try{
			...
			if(empty($msg)){
				$query = "update tbl_news_and_events set title='".$_POST["title"]."',content='".$_POST["content"]."',type='".$_POST["type"]."'$picture where tbl_id='$tbl_id'";
				$con->exec($query);
			?> 
			<script>
				window.location.assign('view-post.php?msg=<p class="w3-green w3-padding-medium w3-margin-0"><i class="fa fa-check"></i> Success updating the post</p>');
			</script>
			<?php
			}
