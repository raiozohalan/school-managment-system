		...
		if($row["rowCount"] == 0){
			...
			if($file_type["mime"] =="image/png" || $file_type["mime"] =="image/jpeg" || $file_type["mime"] =="image/jpg" || $file_type["mime"] == "image/swf" || $file_type["mime"] =="image/bmp" ){
				if(move_uploaded_file($_FILES["picture"]["tmp_name"],$file)){
					$querySave = "insert into tbl_students(fname,lname,mname,gender,bday,address,religion,mtongue,guardian,guardian_contact,guardian_rel,username,password,mother,father)values('".$_POST["fname"]."','".$_POST["lname"]."','".$_POST["mname"]."','".$_POST["gender"]."','".$_POST["bday"]."','".$_POST["address"]."','".$_POST["religion"]."','".$_POST["mtongue"]."','".$_POST["guardian_name"]."','".$_POST["guardian_contact"]."','".$_POST["guardian_rel"]."','".$_POST["username"]."','".md5($_POST["password"])."','".$_POST["mother"]."','".$_POST["father"]."')";
					try{
						$con->exec($querySave);
